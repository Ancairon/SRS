...
def align_set(X_train, y_train_int, input_stl, CLASS_NB, align_iter=5):
    ...
    for cl in range(CLASS_NB):
        ...
        X_set = X_train[index]
        X_set_aligned = tsd.optimize_alignment_to_pattern(
            input_stl.pattern[cl], X_set, iterations=align_iter)
        X_train_aligned = np.concatenate(
            [X_train_aligned, X_set_aligned], axis=0)
        y_train_aligned_int = np.concatenate(
            [y_train_aligned_int, cl*np.ones(len(index), dtype=np.int)])
...
def main(argv):
    ...
    if os.path.isfile(path_dict['stl']+FLAGS.dataset_name+"_STL_decomp.pkl"):
        input_stl = pkl.load(
            open(path_dict['stl']+FLAGS.dataset_name+"_STL_decomp.pkl", 'rb'))
    else:
        input_stl = STL.STL_decomp(SEG_SIZE, CHANNEL_NB, X_train, y_train_int)
        pkl.dump(input_stl, open(
            path_dict['stl']+FLAGS.dataset_name+"_STL_decomp.pkl", 'wb'))

    # Align X_train
    X_train, y_train_int, y_train = align_set(
        X_train, y_train_int, input_stl, CLASS_NB, align_iter=FLAGS.align_iter)
    ...
    cvae_model_path = path_dict['cvae_align']+"CVAE_" + \
        FLAGS.dataset_name+"_"+FLAGS.arch+"_weights"
    ...
    if os.path.isfile(path_dict['stl_align']+FLAGS.dataset_name+"_STL_decomp.pkl"):
        input_stl = pkl.load(
            open(path_dict['stl_align']+FLAGS.dataset_name+"_STL_decomp.pkl", 'rb'))
    else:
        input_stl = STL.STL_decomp(SEG_SIZE, CHANNEL_NB, X_train, y_train_int)
        pkl.dump(input_stl, open(
            path_dict['stl_align']+FLAGS.dataset_name+"_STL_decomp.pkl", 'wb'))
    ...
    X_test, y_test_int, y_test = align_set(
        X_test, y_test_int, input_stl, CLASS_NB, align_iter=FLAGS.align_iter)
    residuals_test, res_labels_test = input_stl.residuals_of(
        X_test, y_test_int)
    ...
    rescvae_model_path = path_dict['rescvae_align'] + \
        "CVAE_"+FLAGS.dataset_name+"_"+FLAGS.arch+"_weights"
    ...
    ll_rem_in = rescvae.likelihood(
        residuals_train, res_labels_hot,  mc_range=50)

    ratio1 = ll_x_in/ll_rem_in
    pkl.dump([ratio1, ll_x_in, ll_rem_in], open(
        path_dict['results_align']+FLAGS.dataset_name+"_res.pkl", 'wb'))

    ll_x_in_test = cvae.likelihood(X_test, y_test,  mc_range=50)
    ll_rem_in_test = rescvae.likelihood(
        residuals_test, res_labels_test_hot,  mc_range=50)

    ratio_test = ll_x_in_test/ll_rem_in_test
    pkl.dump([ratio_test, ll_x_in_test, ll_rem_in_test], open(
        path_dict['results_align']+FLAGS.dataset_name+"_res_onTest.pkl", 'wb'))
    ...
    for i in range(0, len(ood_X_train_original), 10):
        ...
        if "audio_classification" in path \
            ...
            ood_X_train = np.expand_dims(ood_X_train, axis=1)
            # Adding a dimension after samples
        ...
        ood_x, ood_y_int, ood_y = align_set(
            ood_X, ood_y_int, input_stl, CLASS_NB, align_iter=FLAGS.align_iter)  # Align
        ...
        ll_x_ood_rem = rescvae.likelihood(
            residuals_ood, res_ood_labels_hot,  mc_range=50)

        ratio_ood = ll_x_ood/ll_x_ood_rem

        ratio_on_ood[FLAGS.dataset_name_ood] = [ratio_ood, ll_x_ood, ll_x_ood_rem]
        pkl.dump([ratio_on_ood], open(path_dict['results_align'] +
                FLAGS.dataset_name+"_OODres.pkl", 'wb'))

        # Compute AUROC score

        ratio1, _, _ = pkl.load(
            open(path_dict['results_align']+FLAGS.dataset_name+"_res.pkl", 'rb'))
        ratio_test, _, _ = pkl.load(
            open(path_dict['results_align']+FLAGS.dataset_name+"_res_onTest.pkl", 'rb'))
        ratio_in = np.concatenate([ratio1, ratio_test])
        ratio_on_ood = pkl.load(
            open(path_dict['results_align']+FLAGS.dataset_name+"_OODres.pkl", 'rb'))[0]
        ...
        real_auroc_ratio = "%.2f" % au.auroc(
            ratio_in, ratio_elements, ratio_labels)
