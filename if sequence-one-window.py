...
RESULTS_CSV = 'anomaly_eval_single_window.csv'
SEQ_LEN = 10
STEP = 10
THRESHOLD_PCT = 95
SEED = 42
...
def train_isolation_forest(train_windows):
    """
    Fit an IsolationForest on the provided 2D array of training windows.
...
def compute_anomaly_scores(model, windows):
    """
    Given an IsolationForest `model` and a 2D array `windows` (n_samples × n_features),
    return a 1D array of anomaly scores = -score_samples(...), so that higher = more anomalous.
    """
...
if __name__ == "__main__":
    ...
    for train_path in dataset_paths:
        ...
        iso_model = train_isolation_forest(train_windows_scaled)

        scores_train = compute_anomaly_scores(iso_model, train_windows_scaled)
        ...
        for test_path in dataset_paths:
            ...
            score = compute_anomaly_scores(iso_model, single_window_scaled)[0]
            pred = int(score > threshold)   # 1 = “anomaly,” 0 = “normal”
